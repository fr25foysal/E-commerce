...
if ( ! function_exists( 'cartzillagb_block_category' ) ) {
	...
	add_filter( 'block_categories_all', 'cartzillagb_block_category', 10, 2 );
}
...
if ( ! function_exists( 'cartzillagb_rest_get_terms' ) ) {
	/**
	 ...
	 * @since 2.0
	 */
	function cartzillagb_rest_get_terms() {
		$args = array(
			'public' => true,
		);
...
if ( ! function_exists( 'cartzillagb_get_terms_endpoint' ) ) {
	/**
	 * Define our custom REST API endpoint for getting all the terms/taxonomies.
	 *
	 * @since 2.0
	 */
	function cartzillagb_get_terms_endpoint() {
		register_rest_route( 'wp/v2', '/cz_terms', array(
			'methods' => 'GET',
			'callback' => 'cartzillagb_rest_get_terms',
			'permission_callback' => function () {
				return current_user_can( 'edit_posts' );
			},
		) );
	}
	add_action( 'rest_api_init', 'cartzillagb_get_terms_endpoint' );
}
